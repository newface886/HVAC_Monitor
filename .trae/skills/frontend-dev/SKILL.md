...
## 1.1 Design Thinking — Aesthetic Direction

**CRITICAL FIRST STEP:** Before any code, commit to a BOLD aesthetic direction. This is not decoration — it's the soul of the interface.

### Purpose Analysis
- What problem does this interface solve? Who uses it?
- What's the emotional context? (trust, excitement, calm, urgency)
- What action should users feel compelled to take?

### Aesthetic Commitment
Pick an extreme and execute it with precision. Intentionality matters more than intensity.

| Direction | Characteristics | Best For |
|-----------|-----------------|----------|
| **Brutally Minimal** | Aggressive whitespace, single accent, typography-first | Luxury, SaaS, portfolios |
| **Maximalist Chaos** | Layered elements, bold colors, kinetic energy | Entertainment, youth brands |
| **Retro-Futuristic** | CRT textures, neon accents, pixel fonts | Gaming, tech nostalgia |
| **Organic/Natural** | Earth tones, flowing shapes, soft shadows | Wellness, sustainability |
| **Luxury/Refined** | Gold accents, serif typography, restrained motion | Fashion, finance, real estate |
| **Playful/Toy-like** | Rounded shapes, bright colors, bouncy animations | Children, food, casual apps |
| **Editorial/Magazine** | Grid-breaking layouts, dramatic typography, editorial spacing | Media, publishing, culture |
| **Brutalist/Raw** | Monospace fonts, visible structure, no decoration | Art, design studios, hackers |
| **Art Deco/Geometric** | Symmetry, metallic accents, geometric patterns | Hotels, cocktails, premium |
| **Soft/Pastel** | Muted tones, gentle gradients, rounded corners | Dating, lifestyle, beauty |

### Differentiation Question
**What makes this UNFORGETTABLE?** What's the one thing someone will remember? Every interface needs a signature moment.

### Anti-Slop Mandate
NEVER converge on generic AI aesthetics:
- **Banned fonts:** Inter, Roboto, Arial, system fonts, Space Grotesk
- **Banned patterns:** Purple gradients on white, centered hero with stock photo, 3-column equal cards
- **Banned colors:** AI purple/blue, oversaturated neons, pure black (#000)

Choose distinctive, characterful fonts that elevate the aesthetic. Pair a display font with a refined body font. Every design must feel genuinely crafted for its specific context.
...
Adapt dynamically based on user requests and chosen aesthetic direction.

## 1.3 Architecture Conventions
...
- **Display fonts:** Choose distinctive, characterful fonts. Use Google Fonts, Fontshare, or custom imports. Match to aesthetic direction.
- **Pairing:** Display font for headings + refined body font. Create tension and harmony.
- **Scale:** Headlines `text-4xl md:text-6xl tracking-tighter`. Body `text-base leading-relaxed max-w-[65ch]`.
- **NEVER** use Serif on dashboards (unless that's the explicit aesthetic).
- **Hierarchy:** Use weight, size, and spacing — not color alone.

### Color & Theme
- **Commit fully** to one cohesive palette. Dominant colors with sharp accents outperform timid, evenly-distributed schemes.
- Max 1 accent color, saturation < 80%.
- Use CSS variables for consistency across the entire application.
- **NEVER** use AI purple/blue. Explore unexpected palettes that match the aesthetic direction.

### Spatial Composition
- **Break the grid** when VARIANCE > 4. Force asymmetric layouts.
- Use unexpected layouts: asymmetry, overlap, diagonal flow, grid-breaking elements.
- Choose between generous negative space OR controlled density — never in-between.
- Cards are optional — sometimes spacing alone creates hierarchy.
...
### States & Interactions
- **ALWAYS** implement: Loading (skeleton), Empty, Error states
- **ALWAYS** include tactile feedback (`scale-[0.98]` on press)
- Forms: Label above input. Error below. `gap-2` for input blocks.

## 1.5 Anti-Slop Techniques
...
- **Texture Overlays:** Add grain, noise, or subtle patterns to break digital sterility
- **Micro-Interactions:** Hover states that surprise, scroll-triggered reveals that delight
...
| Typography | Inter font, oversized H1s, generic system fonts |
| Layout | 3-column equal card rows, floating elements with awkward gaps, centered heroes when VARIANCE > 4 |
| Components | Default shadcn/ui without customization |
| Aesthetic | Any design that could be mistaken for a template — every interface must feel bespoke |

## 1.7 Creative Arsenal
...
**Design Thinking:**
- [ ] Aesthetic direction explicitly chosen and documented
- [ ] "Unforgettable moment" identified for the interface
- [ ] No banned fonts (Inter, Roboto, Arial, system fonts, Space Grotesk)
- [ ] No banned patterns (purple gradients, centered hero with stock photo, 3-column equal cards)
- [ ] Font pairing is distinctive and characterful
- [ ] Color palette is cohesive and unexpected
...
- [ ] Background has depth/texture, not just solid color
- [ ] Spatial composition matches DESIGN_VARIANCE dial
